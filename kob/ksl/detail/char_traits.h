#ifndef KOB_KSL_DETAIL_CHAR_TRAITS_H_
#define KOB_KSL_DETAIL_CHAR_TRAITS_H_
...
namespace detail {
	...
	inline char16_t char_to_lower(char16_t c) {
		if ((unsigned) c <= 0xff) return (char16_t) tolower((uint8_t) c);
		return c;
	}

	inline char32_t char_to_lower(char32_t c) {
		if ((unsigned) c <= 0xff) return (char32_t) tolower((uint8_t) c);
		return c;
	}


	inline char8_t char_to_upper(char8_t c) { return (char8_t) toupper((uint8_t) c); }

	inline char16_t char_to_upper(char16_t c) {
		if ((unsigned) c <= 0xff) return (char16_t) toupper((uint8_t) c);
		return c;
	}

	inline char32_t char_to_upper(char32_t c) {
		if ((unsigned) c <= 0xff) return (char32_t) toupper((uint8_t) c);
		return c;
	}
	...
	template<typename T>
	inline int compare_icase(const T *p1, const T *p2, size_t n) {
		for (; n > 0; ++p1, ++p2, --n) {
	...
	inline const char32_t *find(const char32_t *p, char32_t c, size_t n) {
...
#if defined(KOB_WCHAR_UNIQUE) && KOB_WCHAR_UNIQUE
		...
		template<typename T>
		inline const T *
		char_type_string_find_first_of(const T *p1Begin, const T *p1End, const T *p2Begin, const T *p2End) {
		...
		template<typename T>
		inline const T *
		char_type_string_rfind_first_not_of(const T *p1RBegin, const T *p1REnd, const T *p2Begin, const T *p2End) {
		...
		template<typename T>
		inline const T *
		char_type_string_find_first_not_of(const T *p1Begin, const T *p1End, const T *p2Begin, const T *p2End) {
			for (; p1Begin != p1End; ++p1Begin) {
				const T *pTemp;
				for (pTemp = p2Begin; pTemp != p2End; ++pTemp) {
					if (*p1Begin == *pTemp)
		...
		template<typename T>
		inline const T *
		char_type_string_rfind_first_of(const T *p1RBegin, const T *p1REnd, const T *p2Begin, const T *p2End) {
			for (; p1RBegin != p1REnd; --p1RBegin) {
				for (const T *pTemp = p2Begin; pTemp != p2End; ++pTemp) {
					if (*(p1RBegin - 1) == *pTemp)
						return p1RBegin;
...
#endif // KOB_KSL_DETAIL_CHAR_TRAITS_H_
