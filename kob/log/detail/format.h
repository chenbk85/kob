...
# define FMT_BUILTIN_CLZ(n) fermat::internal::clz(n)
...
# define FMT_BUILTIN_CLZLL(n) fermat::internal::clzll(n)
}
}
#endif
...
{
private:
    FMT_DISALLOW_COPY_AND_ASSIGN(FmtBuffer);
...
protected:
    ...
    FmtBuffer(T *ptr = 0, std::size_t capacity = 0)
...
template <typename T>
template <typename U>
void FmtBuffer<T>::append(const U *begin, const U *end)
{
...
class MemoryBuffer: private Allocator, public FmtBuffer<T>
{
private:
    T data_[SIZE];
...
public:
    explicit MemoryBuffer(const Allocator &alloc = Allocator())
        : Allocator(alloc), FmtBuffer<T>(data_, SIZE)
    {}
    ~MemoryBuffer()
    {
        deallocate();
...
// A fixed-size buffer.
template <typename Char>
class FixedBuffer: public fermat::FmtBuffer<Char>
{
public:
    FixedBuffer(Char *array, std::size_t size): fermat::FmtBuffer<Char>(array, size)
    {}
...
FMT_API void format_windows_error(fermat::Writer &out, int error_code,
                                  fermat::StringRef message) FMT_NOEXCEPT;
#endif

FMT_API void format_system_error(fermat::Writer &out, int error_code,
                                 fermat::StringRef message) FMT_NOEXCEPT;
...
private:
    ...
    FmtBuffer<Char> &buffer_;
    Char *start_;

public:
    FormatBuf(FmtBuffer<Char> &buffer): buffer_(buffer), start_(&buffer[0])
    {
...
# define FMT_WRAP1(func, arg_type, n) \
  ...
  inline void func(arg_type arg1, FMT_GEN(n, FMT_MAKE_ARG)) { \
    const fermat::internal::ArgArray<n>::Type array = {FMT_GEN(n, FMT_MAKE_REF)}; \
    func(arg1, fermat::ArgList( \
      fermat::internal::make_type(FMT_GEN(n, FMT_MAKE_REF2)), array)); \
  }
...
public:
    /**
    \rst
    Constructs a :class:`fermat::SystemError` object with the description
    of the form
    ...
    **Example**::
    ...
    if (!file)
    throw fermat::SystemError(errno, "cannot open file '{}'", filename);
    \endrst
    */
...
such as :class:`fermat::BasicMemoryWriter`.
...
protected:
    ...
    */
    explicit BasicWriter(FmtBuffer<Char> &b): buffer_(b)
    {}
...
public:
    ...
    /**
    \rst
    Constructs a :class:`fermat::BasicMemoryWriter` object moving the content
...
use :class:`fermat::BasicMemoryWriter`.
...
public:
    /**
    \rst
    Constructs a :class:`fermat::BasicArrayWriter` object for *array* of the
    given size.
    \endrst
    */
...
public:
    /**
    \rst
    Constructs a :class:`fermat::WindowsError` object with the description
    of the form
    ...
    **Example**::
    ...
    if (file == HFILE_ERROR) {
    throw fermat::WindowsError(GetLastError(),
...
Example:
print_colored(fermat::RED, "Elapsed time: {0:.2f} seconds", 1.23);
*/
...
std::string message = fermat::sprintf("The answer is %d", 42);
\endrst
*/
...
fermat::fprintf(stderr, "Don't %s!", "panic");
\endrst
*/
...
fermat::printf("Elapsed time: %.2f seconds", 1.23);
\endrst
*/
...
{
private:
    // FmtBuffer should be large enough to hold all digits (digits10 + 1),
...
# define FMT_WRAP(Char, ReturnType, func, call, n, ...) \
  ...
  inline ReturnType func(FMT_FOR_EACH(FMT_ADD_ARG_NAME, __VA_ARGS__), \
    ...
    call(FMT_FOR_EACH(FMT_GET_ARG_NAME, __VA_ARGS__), fermat::ArgList( \
      fermat::internal::make_type(FMT_GEN(n, FMT_MAKE_REF2)), arr)); \
  }
...
fermat::ArgList args) {
fermat::print("{}: {}: ", file, line);
fermat::print(format, args);
}
...
#define FMT_CAPTURE_ARG_(id, index) ::fermat::arg(#id, id)

#define FMT_CAPTURE_ARG_W_(id, index) ::fermat::arg(L###id, id)

/**
\rst
Convenient macro to capture the arguments' names and values into several
``fermat::arg(name, value)``.
...
inline void require_numeric_argument(const Arg &arg, char spec)
{
    if (arg.type > Arg::LAST_NUMERIC_TYPE)
    {
        ...
        FMT_THROW(fermat::FormatError(message));
    }
}
...
void check_sign(const Char *&s, const Arg &arg)
{
    ...
    {
        FMT_THROW(FormatError(fermat::format(
...
/**
\rst
C++11 literal equivalent of :func:`fermat::format`.

**Example**::

using namespace fermat::literals;
...
/**
\rst
C++11 literal equivalent of :func:`fermat::arg`.
